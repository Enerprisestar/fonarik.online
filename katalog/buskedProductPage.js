    ...
    // delete
      ...
      bb.onclick = function (aaaaaaaa) {
        if (item.amount <= 1) {
          ...
          let aaa = JSON.parse(localStorage.availabilityProductInBusked);
          aaa[availabilityProductInBusked.findIndex(function (item) { if (item.id === aaaaaaaa.srcElement.classList[3].replace(/[^0-9]/g, "")) { return true } })].amount -= 1;
          localStorage.availabilityProductInBusked = JSON.stringify(aaa);
...
function addToBasked(identefikatorProductKatalog) {
  ...
  // identefikatorProductKatalog.srcElement.classList[2]
  document.querySelector('body').style.overflow = "hidden";

  if(document.querySelector('.main_wrapper_card__hero_text')===null){
    var identefikatorProductKatalogNumber=identefikatorProductKatalog.srcElement.classList[2].replace(/[^0-9]/g, "");
    ...
    var identefikatorProductKatalogNumberProductPage=identefikatorProductKatalog.srcElement.classList[2].replace(/[^0-9]/g, "");
    availabilityProductInBusked[availabilityProductInBusked.length] = {
